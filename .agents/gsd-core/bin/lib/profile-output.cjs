...
function cmdGenerateClaudeProfile(cwd, options, raw) {
    ...
    if (options.global) {
        ...
        // Read claude_md_path from config; #1098 default is ./.agents/GEMINI.md
        ...
        let configClaudeMdPath = './.agents/GEMINI.md';
        try {
            const config = loadConfig(cwd);
...
function cmdGenerateClaudeMd(cwd, options, raw) {
    ...
    // `.agents/GEMINI.md` (a valid auto-loaded memory location) rather than a
    // repo-root `GEMINI.md`, so generated GSD content does not land next to — or
    // pollute — a hand-crafted repo-root GEMINI.md. An explicit `claude_md_path`
    ...
    try {
        ...
        // GEMINI.md; copilot now resolves to .github/copilot-instructions.md, and
        ...
        // rationale: a Codex/AGENTS-native project must never write to GEMINI.md
    ...
    if (existingContent === null) {
        ...
        // section markers — it is a hand-crafted GEMINI.md/AGENTS.md, not a
